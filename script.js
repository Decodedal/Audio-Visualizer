...
function animate() {
    ...
    // Rainbow Mode Logic
    if (isRainbowMode) {
        rainbowHue += rainbowSpeed * 0.1;
        if (rainbowHue >= 360) rainbowHue = 0;
        barColor = hslToHex(rainbowHue, 100, 50);
...
function handleInteraction(x, y) {
    // 1. Particle Explosion/Disruption
    particles.forEach(p => {
        const dx = p.x - x;
        const dy = p.y - y;
    ...
    // 2. Color Change (if not in rainbow mode)
    if (!isRainbowMode) {
        const randomHue = Math.random() * 360;
        barColor = hslToHex(randomHue, 100, 50);
        barColorInput.value = barColor;
        updateNowPlaying();
    }
}
